...
void draw_ui(font *f, int level) {
    ...
    for (uint32_t i = 0; i < MAX_AMMO; i++) {
        color_t color = (i >= get_player()->ammo_loaded) ? BLACK : WHITE;
    ...
    if (get_player()->ammo_stored > 0) {
        draw_text_shadow(f, VEC2F(start_pos.x + (MAX_AMMO * 20) - 5, 80), 1.2f, WHITE, VEC2F(3, 3), BLACK, "+%d", get_player()->ammo_stored);
    }
