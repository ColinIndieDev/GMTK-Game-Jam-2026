...
void draw_ui(player_t *player, font *f) {
    vec2f start_pos = VEC2F(10, 10);

    for (uint32_t i = 0; i < MAX_AMMO; i++) {
        color_t color = (i >= player->ammo_loaded) ? BLACK : WHITE;
        draw_texture2D(get_sprite_sheet(SPRITE_SHEET_BULLET)->textures, VEC2F(start_pos.x + (i * 20),start_pos.y), VEC2F(150, 150), color, NO_ROTATION);
    }
